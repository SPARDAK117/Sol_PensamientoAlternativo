...
namespace PensamientoAlternativo.Persistance.Repositories
{
    public class HomeContentRepository : IHomeContentRepository
    {
        ...
        public Task<List<Video>> GetVideosAsync(CancellationToken ct) => _context.Videos.Where(video => video.ViewSection <= 1 ).ToListAsync(ct);
        public Task<List<Image>> GetImagesAsync(CancellationToken ct) => _context.Images.Where(img => !img.IsBannerImage && img.ViewSection <= 1).ToListAsync(ct);
        public Task<List<Image>> GetImagesBreakYourLimitsAsync(CancellationToken ct) => _context.Images.Where(img => !img.IsBannerImage && img.ViewSection == 2).ToListAsync(ct);
        public Task<List<Video>> GetVideosBreakYourLimitsAsync(CancellationToken ct) => _context.Videos.Where(video => video.ViewSection == 2).ToListAsync(ct);
